...
export default class TaskView {
  ...
  display(task) {
    ...
    addCommentButton.addEventListener("click", async () => {
      const comment = await API.pushComment(
        task.id,
        JSON.stringify({
      ...
      task.comments = [...task.comments, comment[0]]
    ...
    authorImg.src = `data:image/png;base64,${task.assignee.photo}`
    ...
    authorName.innerText = task.assignee.userName
