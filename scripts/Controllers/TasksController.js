...
export default class TasksController {
    constructor(header, main){
        this._header = header;
        this._main = main;
        this._taskCollection = new TaskCollection(JSON.parse(localStorage.getItem('tasks')) || []);
    ...
    addTask(name, description, priority, assignee, status, isPrivate) {
        this._taskCollection.add(name, description, priority, assignee, status, isPrivate);
        this._update();
    ...
    getFeed(skip=0, top=10, filterConfig={}) {
        ...
        const remaining = this._taskCollection._tasks.length - this._taskPage;
        ...
        button.addEventListener('click', () => {
            this._update(this._taskPage, 10, {});
            this._taskPage += 10;
        ...
        usernameInput.addEventListener('input', () => this._update(skip, top, {assignee:usernameInput.value}));
        priorityInputs.forEach(input => input.addEventListener('change', () => this._update(skip, top, {priority:input.value})));
        statusInputs[0].addEventListener('change', () => this._update(skip, top, {isPrivate: false}));
        statusInputs[1].addEventListener('change', () => this._update(skip, top, {isPrivate: true}));
    ...
    _update(skip=0, top=10, filterConfig={}){
           // this._taskCollection = new TaskCollection(JSON.parse(localStorage.getItem('tasks')) || []); //???
            //console.log(skip, top, filterConfig)
            this._pageTasks = (new TaskCollection (this._pageTasks)).getPage(skip, top, filterConfig)
