...
function updateSubmitButton() {
  ...
  if (login && password && confirmPassword && username) {
    // все поля заполнены
    if (
      validateLogin(login) &&
    ...
    .then((blob) => {
      ...
      reader.onloadend = function () {
        ...
        const result = JSON.stringify({
          ...
          retypedPassword: String(passwordInput.value),
          photo: String(base64data.substring(22)),
        })

        API.registerUser(result).then(() => {
