...
export default class TaskCollection {
  ...
  getPage = async (skip = 0, top = 10, filterConfig = {}) => {
    const tasks = await API.getTasks(skip, top, filterConfig.status)
    this._tasks = tasks.filter((task) => {
      return (
        task.assignee.login.includes(LocalStorage.getLogin()) ||
    ...
    return tasksArrSortDate
      .filter(
        (task) =>
          !config.assignee || task.assignee.login.includes(config.assignee)
      )
      .filter(
        (task) =>
          !config.description || task.description.includes(config.description)
      )
      .filter((task) => !config.status || task.status === config.status)
      .filter((task) => !config.priority || task.priority === config.priority)
      .filter(
      ...
      .filter(
        (task) =>
          !config.dateFrom ||
          task.createdAt.getTime() >= config.dateFrom.getTime()
      )
      .filter(
        (task) =>
          !config.dateTo || task.createdAt.getTime() <= config.dateTo.getTime()
      )
      .splice(skip, top)
  }
  ...
  edit(
    ...
    const editTaskCopy = {
      ...
      isPrivate:
        typeof isPrivateNew === "boolean" ? isPrivateNew : cheskTask.isPrivate,
    }
